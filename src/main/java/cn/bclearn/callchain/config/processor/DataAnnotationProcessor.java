package cn.bclearn.callchain.config.processor;

import cn.bclearn.callchain.config.annotation.EnableCallChain;
...
public class DataAnnotationProcessor extends AbstractProcessor {
    ...
    public Set<String> getSupportedAnnotationTypes(){
        ...
        set.add(EnableCallChain.class.getCanonicalName());
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        try {
            // 返回被注释的节点
            Set<? extends Element> elements = roundEnv.getElementsAnnotatedWith(EnableCallChain.class);
            //存放注释value
            ...
            for (Element e : elements) {
                ...
                if (e.getKind() == ElementKind.CLASS && e instanceof TypeElement) {
                    ...
                    EnableCallChain annotation = t.getAnnotation(EnableCallChain.class);
            ...
            // 生成一个Java源文件
            JavaFileObject sourceFile = filer.createSourceFile("CallChainAspect");
            // 写入代码
            createSourceFile(info,sourceFile.openWriter());
            // 手动编译
            compile(sourceFile.toUri().getPath());
        } catch (IOException e) {
            messager.printMessage(Diagnostic.Kind.ERROR,"Call Chain 处理失败");
        }
        return true;
    }
